...
import com.revature.models.EmailListNoPage;
...
public class InterviewServiceImpl implements InterviewService {
	...
	public Interview addNewInterview(NewInterviewData i) {
		try {
			...
			Interview newInterview = new Interview(0, managerEmail, associateEmail, scheduled, null, null, location,
					null, null, c);
	...
	public Interview addAssociateInput(NewAssociateInput a) {
		...
		AssociateInput ai = new AssociateInput(0, a.getReceivedNotifications(), a.isDescriptionProvided(), temp,
				a.getInterviewFormat(), a.getProposedFormat());
	...
	public Interview setFeedback(FeedbackData f) {
		InterviewFeedback interviewFeedback = new InterviewFeedback(0, new Date(f.getFeedbackRequestedDate()),
				f.getFeedbackText(), new Date(f.getFeedbackReceivedDate()), new FeedbackStatus(1, "Pending"));
			...
			if (I.getFeedback() == null) {
				...
				if (I.getFeedback().getFeedbackReceived() != null) {
					...
					if (I.getFeedback().getFeedbackDelivered() != null) {
						feedbackChart[3]++; // [3] is the number of interviews that received feedback that hasn't been
											// delivered to associate
					} else {
						feedbackChart[4]++; // [4] is the number of interviews that received feedback that has been
											// delivered to associate
					}
				}
			}
		...
		for (Interview i : allUsers) {
			...
			if (i.getAssociateInput() != null)
				...
				if (i.getAssociateInput().getReceivedNotifications() != null) {
					...
					if (i.getAssociateInput().getReceivedNotifications().before(oneDayBefore)
							|| !(i.getAssociateInput().getReceivedNotifications().after(oneDayBefore))) {
						allNotifiedUsers.add(i);
					}
					System.out.println("getScheduled: " + i.getScheduled() + " oneDayBefore: " + oneDayBefore
							+ " Associate: " + i.getAssociateInput().getReceivedNotifications());
					System.out.println(i.getAssociateInput().getReceivedNotifications().before(oneDayBefore) + " vs "
							+ (!(i.getAssociateInput().getReceivedNotifications().after(oneDayBefore))));
				}
		}
	...
	public List<Interview> getAllInterviewsWithin24HourNoticeManager() {
		...
		for (Interview i : allUsers) {
			...
			if (i.getNotified() != null) {
				...
				System.out.println("getScheduled: " + i.getScheduled() + " oneDayBefore: " + oneDayBefore + " Manager: "
						+ i.getNotified());
				System.out.println(
						i.getNotified().before(oneDayBefore) + " vs " + (!(i.getNotified().after(oneDayBefore))));
			}
	...
	public List<InterviewAssociateJobData> getAllJD() {
		List<InterviewAssociateJobData> interviewList = getAllJDNoName();
		
		// get user information
		List<String> userEmails = new LinkedList<>();
		for (InterviewAssociateJobData associate : interviewList) {
			userEmails.add(associate.getAssocEmail());
		}
		List<User> userList = userClient.findByEmailList(new EmailListNoPage(userEmails));
		
		// place users in map for better retrieval(by email)
		Map<String, User> userMap = new TreeMap<>();
		for (User user : userList) {
			userMap.put(user.getEmail(), user);
		}

		// get user names for interviews
		for (InterviewAssociateJobData associate : interviewList) {
			User user = userMap.get(associate.getAssocEmail());
			associate.setAssocName(user.getFirstName() + " " + user.getLastName());
		}
